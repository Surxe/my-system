...
**Where to edit:** once consent is given, the *where-to-write* rule is its own memory: [[memory-files-edit-in-repo]] — edit the repo copy under `users/dev/memory/<project>/` and deploy via `install.sh`, never the live `~/.claude` copies. Consent (this memory) and location (that one) are separate gates; clearing one does not clear the other. Same workflow as [[bashrc-edit-workflow]]; see also [[user-specific-via-my-system]].
