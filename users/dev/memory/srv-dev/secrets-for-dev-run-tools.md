...
**Sudo env whitelist (easy to miss):** `--preserve-env=VAR` alone is NOT enough — sudo's `env_reset` on the ethan→dev bridge strips any var not whitelisted, failing with "you are not allowed to set the following environment variables: VAR". Each secret var must be added to the additive `env_keep` in `my-system/system/etc-sudoers.d/devbridge-env` (`Defaults:ethan env_keep += "VAR"`), then deployed with install.sh (validated via `visudo -cf`). So adding a new dev-run secret is a THREE-part change: secrets.env(.example) + launcher + the sudoers whitelist entry. See `users-and-permissions/sudo-policy.md`.
