...
if [ "$ME" = dev ]; then
    git -C "$DEV_ENV_REPO" pull --ff-only || say "dev-env-layer: WARNING could not update dev-env clone"
else
    sudo -u dev git -C "$DEV_ENV_REPO" pull --ff-only || say "dev-env-layer: WARNING could not update dev-env clone"
fi
